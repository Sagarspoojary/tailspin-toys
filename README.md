...
`predev` migrates and seeds the local database first. Then navigate to the [website](http://localhost:4321) to see the site! Use the homepage filter panel to narrow the catalog by category and publisher before choosing a game to support.
...
Repository-wide coding standards are documented in the [Copilot instructions](.github/copilot-instructions.md) and the focused [instruction files](.github/instructions/). In particular:

- Comments explain intent, constraints, and non-obvious decisions — never code that is already self-explanatory.
- Exported functions in `db/` and `src/lib/` use TSDoc/JSDoc to document purpose, parameters, and return values.
- Reusable Astro components document their `Props` interfaces and public contracts.
- TypeScript formatting is checked by ESLint (single quotes, semicolons, trailing commas, consistent spacing, and no trailing whitespace).
